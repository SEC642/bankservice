...
def usage():
    ...
    print "  --ip #      serve on IP # (default 127.42.84.9)"
    print "  --port #    serve on port # (default 4242)"
...
if __name__ == '__main__':
    ...
    port = DEFAULT_PORT
    ssl = False
    opts, args = getopt.getopt(sys.argv[1:], "", ["ssl", "debug", "help", "ip=", "port="])
    for o, a in opts:
        if o == "--help":
            usage()
            sys.exit(2)
    ...
    server = wsgiserver.CherryPyWSGIServer((ip, port), d, timeout=200)

    print "Serving %s on %s port %d %s" % ("HTTP" if not ssl else "HTTPS", ip, port, "(debug enabled)" if app.debug else "")
